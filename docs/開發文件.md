...
- **開發工具**
  - Nodemon 熱重載
  - Windows 啟動腳本
  - API 測試文檔 (PowerShell/cURL)
  - MongoDB 設置指南

#### 文檔和架構 ✅
- 完整的開發文檔 (docs/開發文件.md)
- 後端 README 和 API 測試指南
- MongoDB 設置教程 (3 種方案)
- 服務層架構說明
- Git 版本控制和 GitHub 集成
...
│   ├── controllers/             # 業務邏輯控制器
│   │   ├── authController.js    # 認證 (註冊/登入)
│   │   ├── userController.js    # 用戶管理
│   │   ├── postController.js    # 文章 CRUD
│   │   └── commentController.js # 評論管理
│   ├── middlewares/             # 中間件
│   │   ├── auth.js              # JWT 認證
...
const devConfig = {
  dev: {
    useDummyData: true,  // 改為 false 使用真實後端
  },
  api: {
...
**API 服務層** (`frontend/src/services/*.api.ts`)
- 處理後端包裝的響應格式
- 自動提取 `response.data` 中的實際數據
- 統一錯誤處理

**切換步驟**
1. 確保 MongoDB 已連接 (本地或 Atlas)
2. 啟動後端: `cd backend && npm run dev`
3. 修改前端配置: `useDummyData: false`
4. 啟動前端測試
...
**測試 API**
- 使用 PowerShell (見 `backend/API_TESTING.md`)
- 使用 Postman 或類似工具
- 使用前端應用直接測試
